export default function showInformation(player, div, current) {
  ...
  for (let i = 0; i < player.units.length; i++) {
    ...
    nameDivs[i].innerHTML = player.units[i].name;
  ...
  if (current && player.units.includes(current)) {
    ...
    infoCurrent.children[0].children[0].src = `./assets/ui/characters/${player.name}/${player.units.indexOf(
      current
    )}.png`;
    infoCurrent.children[1].innerHTML = current.name;
    infoCurrent.children[2].innerHTML = `${current.health < 0 ? 0 : current.health} / ${current.maxHealth}`;
  }
}
